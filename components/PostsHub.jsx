...
// ---------------------------------------------------------------------------
// صف اللايك + عداد التعليقات — يستخدم في الكارت (فييد/جريد) وفي صفحة المنشور
// ---------------------------------------------------------------------------
function EngagementRow({ postId, liked, likeCount, commentCount, onToggleLike, size = "sm", hideComment = false }) {
  const big = size === "lg";
  return (
    <div className="flex items-center gap-4" onClick={(e) => e.stopPropagation()}>
      <button
        onClick={() => onToggleLike && onToggleLike(postId)}
        ...
        aria-label="لايك"
      >
        <span style={{ fontSize: big ? 18 : 15 }}>{liked ? "❤️" : "🤍"}</span>
        {likeCount > 0 ? likeCount : ""}
      </button>
      {!hideComment && (
        <span className="flex items-center gap-1.5 font-bold" style={{ fontSize: big ? 14 : 12, color: "var(--text-muted)" }}>
          <span style={{ fontSize: big ? 18 : 15 }}>💬</span> {commentCount > 0 ? commentCount : ""}
        </span>
      )}
    </div>
  );
...
// ---------------------------------------------------------------------------
function PostCard({ post, onOpen, delay = 0, variant = "grid", showStats = true, liked = false, likeCount = 0, commentCount = 0, onToggleLike }) {
  if (variant === "feed") {
      ...
      <Reveal delay={delay}>
        ...
        >
          ...
          <div className="p-4 sm:p-5 flex-1 min-w-0">
            ...
            {post.excerpt && (
              <p className="text-sm leading-relaxed line-clamp-2 mb-2" style={{ color: "var(--text-secondary)" }}>{post.excerpt}</p>
            )}
            {showStats && (
              <EngagementRow postId={post.id} liked={liked} likeCount={likeCount} commentCount={commentCount} onToggleLike={onToggleLike} />
            )}
          </div>
        </div>
      </Reveal>
  ...
  return (
    <Reveal delay={delay}>
      <div
        ...
        tabIndex={0}
        onKeyDown={(e) => { if (e.key === "Enter") onOpen(post.id); }}
        className="w-full text-right card-pro rounded-2xl overflow-hidden h-full flex flex-col"
        style={{ cursor: "pointer" }}
      >
...
// ---------------------------------------------------------------------------
export function PostsFeedSection({ onOpenPost, onViewAll, content = {} }) {
  ...
  useEffect(() => {
    ...
    fetchPublished("sitePosts", { limit: 5 })
      .then((data) => {
        ...
        fetchLikesFor(ids, voterKey.current)
    ...
    <section dir="rtl" className="px-6 md:px-10 py-16 md:py-20" style={{ background: "var(--bg-elevated)" }}>
      <div className="max-w-3xl mx-auto">
        <Reveal className="text-center mb-10">
          <SectionLabel>{content.eyebrow || "LATEST UPDATES"}</SectionLabel>
          <h2 className="text-2xl md:text-4xl font-black tracking-wide mt-3 mb-3" style={{ fontFamily: "var(--font-cinzel), serif", color: "var(--text-primary)" }}>
            {content.title || "أحدث المنشورات"}
          </h2>
          <p className="text-sm md:text-base" style={{ color: "var(--text-secondary)" }}>
            {content.subtitle || "نصائح، عروض، وأخبار بنشاركها معاك أول بأول"}
          </p>
        </Reveal>
...
// ---------------------------------------------------------------------------
// صندوق التعليقات — قائمة التعليقات المعتمدة + فورم إضافة تعليق جديد
// ---------------------------------------------------------------------------
function CommentsBlock({ postId }) {
  ...
  return (
    <div>
      <h3 className="text-lg font-black mb-5" style={{ color: "var(--text-primary)", fontFamily: "var(--font-cinzel), serif" }}>
        💬 التعليقات {comments ? `(${comments.length})` : ""}
      </h3>

      <form onSubmit={handleSubmit} className="mb-8">
        <input value={authorName} onChange={(e) => setAuthorName(e.target.value)} placeholder="اسمك" style={inputStyle} maxLength={40} required />
        <textarea value={commentText} onChange={(e) => setCommentText(e.target.value)} placeholder="اكتب تعليقك..." rows={3} style={{ ...inputStyle, resize: "vertical" }} maxLength={500} required />
        {error && <p className="text-xs mb-3" style={{ color: "#ff6a6a" }}>{error}</p>}
        <button
          type="submit"
          disabled={posting}
          className="font-black px-6 py-2.5 rounded-xl text-black text-sm transition duration-300"
          style={{ background: `linear-gradient(135deg, ${GOLD}, var(--gold-light))`, opacity: posting ? 0.6 : 1, cursor: posting ? "default" : "pointer" }}
        >
      ...
      {comments === null ? (
        <p className="text-xs" style={{ color: "var(--text-muted)" }}>جاري تحميل التعليقات...</p>
      ) : comments.length === 0 ? (
        <p className="text-xs" style={{ color: "var(--text-muted)" }}>كن أول من يعلّق على هذا المنشور.</p>
      ) : (
          ...
          {comments.map((c) => (
            <div key={c.id} className="flex gap-3">
              <div
                ...
                style={{ width: 34, height: 34, borderRadius: "50%", background: "rgba(201,150,58,0.15)", color: GOLD, fontSize: 13 }}
              >
                {(c.author_name || "?")[0]}
              </div>
              <div>
                <p className="text-sm leading-relaxed">
                  <span className="font-bold" style={{ color: "var(--text-primary)" }}>{c.author_name}</span>{" "}
                  <span style={{ color: "var(--text-secondary)" }}>{c.comment_text}</span>
                </p>
                <p className="text-[11px] mt-0.5" style={{ color: "var(--text-muted)" }}>{formatDate(c.created_at)}</p>
              </div>
            </div>
          ))}
        </div>
...
export function PostDetailPage({ postId, onBack, onOpenPost }) {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      .then(([p, all]) => {
        ...
        fetchLikesFor([postId], voterKey.current)
          .then(({ counts, likedByMe }) => {
            if (!alive) return;
            setLikeCount(counts[postId] || 0);
            setLiked(!!likedByMe[postId]);
          })
          .catch(() => {});
  ...
  async function handleToggleLike() {
    ...
    try {
      await toggleLike(postId, voterKey.current, wasLiked);
    } catch {
      setLiked(wasLiked);
      setLikeCount((c) => c + (nextLiked ? -1 : 1));
    }
    ...
    >
      <div className="max-w-3xl mx-auto">
        ...
        <div className="flex items-center justify-between flex-wrap gap-4 mb-6">
          <EngagementRow postId={post.id} liked={liked} likeCount={likeCount} commentCount={0} onToggleLike={handleToggleLike} size="lg" hideComment />
        ...
        {related.length > 0 && (
          <div className="mt-16">
            ...
            <div className="grid sm:grid-cols-3 gap-5">
              {related.map((p, i) => (
                <PostCard key={p.id} post={p} onOpen={onOpenPost} delay={i * 60} variant="grid" showStats={false} />
              ))}
            </div>
          </div>
        )}
